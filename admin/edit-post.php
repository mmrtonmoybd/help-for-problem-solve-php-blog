...
<head>
    ...
    <title>Needed MCQS - Edit Post</title>
</head>
...
if(isset($_GET['edit'])){
    ...
    $get_run = mysqli_query($connection, $get_query);
        ...
        $get_query = "SELECT * FROM posts WHERE id = $edit_id and author_username = '$session_username'";
        $get_run = mysqli_query($connection, $get_query);
    }
    ...
    <?php require_once('inc/header.php'); ?>
        ...
        <div class="container-fluid body-section">
            <div class="row">
                ...
                <div class="col-md-9">
                    <h2>
                        <li class="fa fa-pencil"></li> Edit Post <small style="font-size:25px;" class="text-muted"> Edit Post Details</small>
                    </h2>
                    <hr>
                ...
                if(isset($_POST['update'])){
                    ...
                    $up_category = $_POST['categories'];
                    $up_status = $_POST['status'];

                    

                    if(empty($up_title) or empty($up_post_data) or empty($up_tags) or empty($up_category) or empty($up_status)){
                        $error = "All (*) fields are required";
                    }else{
                        $update_query = "UPDATE `posts` SET `date` = current_timestamp(), `title` = '$up_title', `category` = '$up_category', `tags` = '$up_tags', `post_data` = '$up_post_data', `statuss` = '$up_status' WHERE `posts`.`id` = $edit_id;";
                   ...
                   <div class="col-md-9">
                    ...
                    if(isset($error)){
                        ...
                        ?>
                            ...
                            <input type="text" name="title" class="form-control" placeholder="Type here title..." value="<?php if(isset($title)){echo $title; } ?>" id="title" >
                            </div>

                            
                            <div class="form-group">
                            <label for="post-data">Post Content:*</label>
                            <textarea name="post-data" id="post-data" cols="30" rows="10" value="" class="form-control" ><?php if(isset($post_data)){echo $post_data; } ?></textarea>
                            </div>
                            ...
                            <textarea name="tags" id="tags" cols="30" rows="10" class="form-control" value="asdasd" ><?php if(isset($tags)){echo $tags; } ?></textarea>
                            </div>
                                </div>
                                <div class="col-sm-6">
                                <div class="form-group">
                                ...
                                <select name="categories" id="categories" class="form-control">
                                   ...
                                   if(mysqli_num_rows($cat_run) > 0){
                                    ...
                                    while($cat_row = mysqli_fetch_array($cat_run)){
                                        ...
                                        echo "<option value='".$cat_name."'".((isset($category) && $category == $cat_name)? "selected":"").">".ucfirst($cat_name)."</option>";
                                    }
